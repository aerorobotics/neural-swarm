...
class CollectData:
    ...
    def _connected(self, link_uri):
        ...
        self._file.write("time[ms],weight[g],pwm,vbat[V]\n");
    ...
    def _stab_log_data(self, timestamp, data, logconf):
        ...
        self._file.write("{},{},{},{}\n".format(timestamp,data['loadCell.weight'], data['pwm.m1_pwm'], data['pm.vbat']))
        # pass
    ...
    #         self._cf.commander.send_setpoint(0, 0, 0, int(thrust))
    #         print(thrust)
    #         time.sleep(0.1)
    ...
    def _ramp_motors(self):
        ...
        for w in [5, 10, 20]:
            for t in np.arange(0, 5, 0.01):
                thrust = (np.sin(t*w) + 1.25) / 2.25 * 65535
                print(thrust)
                self._cf.commander.send_setpoint(0, 0, 0, int(thrust))
                time.sleep(0.01)
        ...
        for t in np.arange(0, 10, 0.01):
            thrust = np.clip(thrust + np.random.normal(0, stddev), 15000, 65535)
            print(thrust)
            self._cf.commander.send_setpoint(0, 0, 0, int(thrust))
            time.sleep(0.01)
