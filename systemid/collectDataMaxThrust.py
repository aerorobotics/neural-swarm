...
class CollectData:
    ...
    def _ramp_motors(self):
        ...
        while self.is_connected: #thrust >= 0:
            ...
            self.measurements = []
            self._applyThrust(thrust, 1.0)
            achievedThrust = np.min(np.array(self.measurements)[:,0])
            vbat = np.mean(np.array(self.measurements)[:,1])

            # go to full thrust
            self.measurements = []
            self._applyThrust(65535, 1.0)
            maxThrust = np.max(np.array(self.measurements)[:,0])
            vbatAtMaxThrust = np.mean(np.array(self.measurements)[:,1])

            # write result
            print(achievedThrust, thrust, vbat, maxThrust, vbatAtMaxThrust)
            self._file.write("{},{},{},{},{}\n".format(achievedThrust, thrust, vbat, maxThrust, vbatAtMaxThrust))
