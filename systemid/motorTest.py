...
class CollectData:
    ...
    def _stab_log_data(self, timestamp, data, logconf):
        ...
        self.measurements.append(data['loadCell.weight'])
    ...
    def _ramp_motors(self):
        ...
        for motor in [(65000,0,0,0),(0,65000,0,0),(0,0,65000,0),(0,0,0,65000), \
                      (65000,0,65000,0),(0,65000,0,65000), \
                      (65000,65000,0,65000), \
                      (65000,65000,65000,65000)]:
            self._cf.param.set_value('motorPowerSet.m1', str(motor[0]))
            self._cf.param.set_value('motorPowerSet.m2', str(motor[1]))
            self._cf.param.set_value('motorPowerSet.m3', str(motor[2]))
            self._cf.param.set_value('motorPowerSet.m4', str(motor[3]))

            time.sleep(1.0)
            self.measurements = []
            time.sleep(2.0)
            print(motor, np.mean(self.measurements))
