...
def consistency_check(robot, x, u, dt, data_neighbors, trust_fa=2):
  print("Running consistency check...", robot.cftype, robot.x_min, robot.x_max)
  T = x.shape[0]
  ...
  for t in range(0, T):
    ...
    if abs(Fa.detach().numpy() - x[t,4].numpy()) > robot.trust_Fa(robot.cftype):
      print("Warning: Fa estimate different from its state at t {}: predicted: {:.2f} state: {:.2f}".format(t, Fa.detach().numpy(), x[t,4]))
    ...
    for cftype_neighbor, xn in data_neighbors_t:
      ...
      if dist < min_dist:
        print("Warning: Collision with neighbor at t {}: is: {:.2f} limit: {:.2f}".format(t, dist, min_dist))
    ...
    if t < T - 1:
      ...
      if (diff > torch.tensor([0.01,0.01,0.05,0.05,robot.trust_Fa(robot.cftype)])).any():
        print("Warning: bad state propagation at t {}: {}".format(t, diff))

    # check state bounds
    if (x[t] < torch.tensor(robot.x_min)).any() or (x[t] > torch.tensor(robot.x_max)).any():
        print("Warning: state out of bounds at t {}: {}".format(t, x[t]))

    # check control bounds
    if t < T - 1:
      if torch.norm(u[t]) > robot.g*robot.thrust_to_weight:
          print("Warning: control out of bounds at t {}: {}".format(t, u[t]))
      ...
      if robot.useNN:
        ...
        for k, (cftype_neighbor, x_neighbor) in enumerate(data_neighbors_t):
          ...
          if abs(Fa_neighbor.detach().numpy()) > robot.max_Fa(cftype_neighbor):
            print("Warning: neighbors Fa is out of bounds at t {}: predicted: {:.2f} state: {:.2f}".format(t, Fa_neighbor.detach().numpy(), x_neighbor[4]))
          if abs(Fa_neighbor.detach().numpy() - x_neighbor[4].numpy()) > robot.trust_Fa(cftype_neighbor):
            print("Warning: neighbor Fa estimate different from its state at t {}: predicted: {:.2f} state: {:.2f}".format(t, Fa_neighbor.detach().numpy(), x_neighbor[4]))
